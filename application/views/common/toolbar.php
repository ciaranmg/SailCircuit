			...
			<? if($this->userlib->check_permission(str_replace('#', '', $button['action']))): ?>
					<? if(!isset($button['type']) OR $button['type'] == 'link'): 
						if(isset($button['url'])) {
							...
							$action = base_url($button['action']) .'/' .$button['parameters'];
						}
					?>
						<a href="<?=$action;?>" class="btn <?=$button['classes']?>" title="<?=$button['title'];?>" <? if(isset($button['attributes'])) echo $button['attributes'];?> >
